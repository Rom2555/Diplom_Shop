...
from shop_app.serializers import AddToBasketSerializer, BasketSerializer, ConfirmOrderSerializer, OrderSerializer
...
class BasketAPIView(APIView):
    ...
    @extend_schema(summary="Получить текущую корзину", responses={200: BasketSerializer})
    def get(self, request, *args, **kwargs):
        # Поиск статуса 'basket' у пользователя
        basket = (
            Order.objects.filter(user=request.user, state="basket").prefetch_related("ordered_items__product").first()
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not product.category.shop.state:
                return Response({"Status": False, "Errors": "Магазин временно не принимает заказы"}, status=400)

            # Проверка что продукт относится к этому же магазину
            if product.category.shop_id != shop_id:
                return Response({"Status": False, "Errors": "Этот товар не принадлежит данному магазину"}, status=400)
            ...
            item = OrderItem.objects.filter(order=basket, product=product, shop_id=shop_id).first()
...
class BasketDeleteView(APIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            item = OrderItem.objects.get(id=items_id, order__user=request.user, order__state="basket")
            item.delete()
    ...
    def post(self, request, *args, **kwargs):
        ...
        for item in basket.ordered_items.all():
            if item.product.quantity < item.quantity:
                return Response(
                    {"Status": False, "Errors": f"Товар {item.product.name} закончился на складе"}, status=400
                )

        # Атомарное списание товаров в БД
        for item in basket.ordered_items.all():
            updated = Product.objects.filter(id=item.product_id).update(quantity=F("quantity") - item.quantity)
